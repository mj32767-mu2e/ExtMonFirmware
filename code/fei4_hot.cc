...
//  Now set up the receiver...
  ...
  do {
    ...
    for ( int i=0; i<len/4; i++ ) {
      ...
      if ( (word&0xffffff) != 0 && (word&0xf0000000) == 0 ) {
        unsigned int ich = (word>>24)&0x0f;
        unsigned int ir = (word>>8)&0x1ff;
        unsigned int ic = (word>>17)&0x7f;
        pixel_address pixel = { ich, ic, ir };
        counts[pixel] += 1;
      }
    }
    if ( t1 != t2 ) {
      cout << "Received " << total << " bytes." << endl;
    }
    t2 = t1;
  }
  while ( t1-t0 < 10 );
  ...
  fei4->ClearBuffer();
  fei4->Buffer()->Write(FEI4_CMD(2*chan),0xaa070000);   // Conf
  fei4->WriteBuffer();
 
//  Now set up the receiver...

  multimap<int,pixel_address> inv;
  for ( map<pixel_address,int>::const_iterator i=counts.begin(); i!=counts.end(); i++ ) {
    inv.insert(make_pair(i->second,i->first));
  } 
  
  for ( multimap<int,pixel_address>::const_reverse_iterator i=inv.rbegin(); i!=inv.rend(); i++ ) {
    if ( i->first > 1 ) {
      cout << "Chan " << i->second.chan << "  r" << i->second.row-1 << "c" << i->second.col-1 << "  rate =  " << (i->first/10.0) << " Hz" << endl;
    }
  }
