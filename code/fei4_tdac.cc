...
//
        ...
        for ( int j=0; j<14; j++ ) {
          ...
          for ( vdac[j][1] = vdac[j][0]; vdac[j][1] < vdac[j][2]; vdac[j][1] += 32 ) {
            ...
            for ( int k=0; k<14; k++ ) {
              ...
              cout << "k = " << k << " eff = " << eff << endl;
              cout << "vdac[" << k << "] = [" << vdac[j][0] << "," << vdac[j][1] << "," << vdac[j][2] << "]" << endl;
            }
          }
  ...
  if ( fout != NULL ) {
    char filename[32];
    sprintf( filename, "cal_%d_ch%d.dat", vthrc*256+vthrf, chan );
    ofstream of(filename);
    int tvdac = 175;
    time_t t0;
    time(&t0);
    of << "Date=" << ctime(&t0);
    of << "Vthr=" << vthrc*256+vthrf << endl;
