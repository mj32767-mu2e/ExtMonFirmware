...
int main(int argc,char **argv) {
  ...
  fei4->WriteGlobalRegister(chan,"Trig_Count",2);
  ...
  double eff = (double)(counters[1]&0xffff)/ntrig;
  double eeff = sqrt(eff*(1-eff)/ntrig);
  cout << "   vdac = " << vdac << ", eff = " << fixed << setprecision(3) << eff << " +- " << fixed << setprecision(3) << eeff << endl;
}
