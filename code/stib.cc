...
const char *Stib::_opname[] = { "nop", "write", "set", "ack", "read", "reset", "default", "waitclr", "waitset" };
...
int Stib::Write(unsigned int addr,unsigned int data) {
  ...
  *_pbuf = 0x00;
  ...
  if ( _pbuf - _txbuf >= BUFLEN ) {
    std::ostringstream os;
    os<<__func__<<": BUFLEN="<<BUFLEN<<" overflow.  Last addr= \n";
...
int Stib::SetBits(unsigned int addr,unsigned int data) {
  *_pbuf++ = _iseq;
  *_pbuf++ = SET;
  *_pbuf++ = 0;
  *_pbuf++ = 8;
...
int Stib::ClearBits(unsigned int addr,unsigned int data) {
  *_pbuf++ = _iseq;
  *_pbuf++ = CLEAR;
  *_pbuf++ = 0;
  *_pbuf++ = 8;
