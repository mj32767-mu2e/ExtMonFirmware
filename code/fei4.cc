...
Fei4Interface::Fei4Interface(const char *addr,const char *file) :
  _log(0), _filename(file) {

  for ( int i=0; i<12; i++ ) _cal[i] = NULL;
    ...
    while ( getline(ifs,line) ) {
      ...
      while ( s.good() ) {
        ...
        if ( token.compare("Channel") == 0 ) {
          std::string val;
          s >> val;
          ichan = atoi(val.c_str());
          if ( ichan < 0 || ichan > 11 ) {
            cout << "Line " << iline << " : Bad channel " << ichan << endl;
            ierr = 1;
          }
          continue;
        }
        if ( token.compare("Load") == 0 ) {
          ...
          for ( int i=0; i<12; i++ ) {
            if ( ichan < 0 || i == ichan ) {
              cout << "Loading channel " << ichan << " calibrations '" << val << "'..." << endl;
              _cal[ichan] = new Fei4Calibrations(val.c_str());   
            }
          }
          continue;
        }
...
//  xx 1010 0000 111000
  _msg->Write(FEI4_CMD(chan),0xaa380000);      //  Run both chips
  _msg->WaitClear(FEI4_CMD(chan),0x80000000);
  _msg->Send();
  if ( ( ierr = _msg->Receive() ) < 0 ) {
...
//  usleep(200);
  ...
  unsigned int word = 0x80000008 | (dly<<16) | (n<<8);
  _msg->Write(0xc8000004,word);
  _msg->WaitClear(0xc8000004,0x80000000);
  _msg->WaitClear(FEI4_CMD(chan),0x80000000,4*n);
//  _msg->Write(FEI4_CMD(chan),0xaa070000);      // Conf both chips
//  _msg->Write(FEI4_CMD(chan),0xa8070000|(chipid<<22));  // Conf
// xx 1010 0000 000111
  _msg->Send();
  if ( ( ierr = _msg->Receive() ) < 0 ) {
    cerr << "Pulse() receive() - timeout." << endl;
