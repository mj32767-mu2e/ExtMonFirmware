...
namespace fei4 {
  ...
  inline unsigned int FEI4_RDREG(int i) { return 0xc8001010+0x100*(i); }
...
//      _gr[28] |= 0x8011;
  ...
  public:
    Fei4Calibrations(const char *file) :
      _filename(file), _vthr(0), _trimdac(0), _dx(3), _tx(3,2) {
      ...
      while ( getline(ifs,line) ) {
        ...
        while ( s.good() ) {
          ...
          if ( e != std::string::npos ) {
            ...
            else if ( key.compare("Dx") == 0 ) {
              _dx[0] = strtod(val.c_str(),NULL);
            }
            else if ( key.compare("Dy") == 0 ) {
              _dx[1] = strtod(val.c_str(),NULL);
            }
            else if ( key.compare("Dz") == 0 ) {
              _dx[2] = strtod(val.c_str(),NULL);
            }
            else if ( key.compare("RotZ") == 0 ) {
              double ang = strtod(val.c_str(),NULL);
    ...
    TVectorT<double> Alignment(int ic,int ir) const {
      TVectorT<double> px(2);
      px[0] = ic;
      px[1] = ir;
      return _tx*px+_dx;
    }
  ...
  class Fei4Interface {
    ...
    Fei4Calibrations *_cal[12];
    const char *_filename;
    int _log;
  public:
    Fei4Interface(const char *,const char *f=NULL);
    const Fei4Calibrations *Calibrations(int i) const { return _cal[i]; };
    void WriteModifiedRegisters(int c) {
      WriteModifiedRegisters(c,_gr[c]);
    }
    stib::Stib *Buffer() { return _msg; }
    const GlobalRegisters *Registers(int ichan) const { return _gr+ichan; }
